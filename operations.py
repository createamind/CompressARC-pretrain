...
class OperationLibrary(nn.Module):
    ...
    def __init__(self):
        ...
        # 操作嵌入层 - 将维度从64改为128，匹配规则嵌入维度
        self.operation_embedding = nn.Embedding(len(self.operations), 128)  # 改成128维而不是64维

        # 操作序列编码器 - 同样需要更新输入维度
        self.sequence_encoder = nn.GRU(128, 128, batch_first=True)  # 改成128维输入
