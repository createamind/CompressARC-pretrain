...
def load_model(model_path, device='cuda', use_discrete_vae=True,
              codebook_size=512, embedding_dim=64, use_hierarchical_vae=False, use_residual=True):
...
def ensemble_prediction(model, input_tensor, num_steps_list=[1, 3, 5], device='cuda', use_discrete_vae=True,  use_hierarchical_vae=False):
    ...
    with torch.no_grad():
        ...
        for steps in num_steps_list:
            if use_hierarchical_vae:
                # 层次化VAE使用直接前向传播推理
                recon_batch, _, _, _, _, _ = model(input_tensor.to(device))
                indices = torch.argmax(recon_batch, dim=-1)
                output = indices.reshape(-1, 30, 30)
            else:
                # 使用现有模型的reconstruct方法
...
if __name__ == "__main__":
    ...
    if args.param_scan:
        ...
        parameter_scan(
            args.model,
            args.data,
            use_discrete_vae=not args.standard_vae and not args.hierarchical_vae,
        ...
        evaluate_all_tasks(
            ...
            args.tt_epochs,
            not args.no_amp,
            not args.standard_vae and not args.hierarchical_vae,
            args.hierarchical_vae,
            args.codebook_size,
            args.embedding_dim,
            args.gpu
        )