...
def plot_results(task_id, input_grid, output_grid, predicted_grid, save_path):
    ...
    # 只显示非零部分
    input_h, input_w = np.where(input_colors > 0)
    if len(input_h) > 0:
        max_h, max_w = max(input_h) + 1, max(input_w) + 1
        input_colors = input_colors[:max_h, :max_w]
    else:
        input_colors = input_colors[:5, :5]  # 空网格显示较小区域
    
    output_h, output_w = np.where(output_colors > 0)
    if len(output_h) > 0:
