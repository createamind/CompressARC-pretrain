...
# 添加到现有utils/data_utils.py文件底部

def arc_grid_collate_fn(batch):
    """
    自定义的collate函数，处理不同大小的ARC网格
    ...
    grid_types = [item['grid_type'] for item in batch]
    ...
    # 将填充后的网格和掩码堆叠成批次
    padded_grids = torch.stack(padded_grids)
    padding_masks = torch.stack(padding_masks)

    return {
        'grid': padded_grids,
        'mask': padding_masks,  # 添加掩码区分填充和实际数据
...
def create_arc_dataloader(data_path, batch_size=32, shuffle=True, num_workers=4,
    ...
    train_loader = DataLoader(
        ...
        num_workers=num_workers,
        pin_memory=True,  # 加速CPU->GPU传输
        collate_fn=arc_grid_collate_fn  # 添加自定义collate函数
    )
