...
class ObjectComposer(nn.Module):
    ...
    def forward(self, objects, pixel_features, grid_size):
        """组合对象和像素特征"""
        # 创建对象特征图
        batch_size = pixel_features.size(0)
        object_map = torch.zeros(batch_size, 128, grid_size[0], grid_size[1], device=pixel_features.device)
        
        # 简化：使用平均对象特征填充地图
        if objects:
            obj_mean = torch.cat(objects, dim=0).mean(dim=0, keepdim=True)
            object_map[:] = obj_mean.view(1, 128, 1, 1).expand(-1, -1, grid_size[0], grid_size[1])
